...
private void initializeControls(){
			...
			if (!cd.isConnectingToInternet()) {
				...
				try{
					...
					JsonObjectRequest  jsonArrayRQST = new JsonObjectRequest(Request.Method.POST,url, null,  new Response.Listener<JSONObject>() {
				        @Override
				        public void onResponse(JSONObject response) {//in response we get response from server
				             gotLoginResponce(response);//passing response in gotLoginResponce
				        }
				    }, new ErrorListener() {
	...
	private void gotLoginResponce(JSONObject jsonObj){
		...
		if(jsonObj != null){
			...
			Log.i("SUSHIL", "JSON--> "+jsonObj.toString());
				try {
